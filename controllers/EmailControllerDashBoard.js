...
const emailOnNewActivity = async (req, res) => {
  const {
    affiliateType,
    websiteUrl,
    postID,
...
const sendEmailOnBlogPostApproval = async (req, res) => {
  ...
  if (los) {
    const titleWithoutDashes = post_title_query?.replace(/-/g, " ");
...
const sendEmailForActivityApproval = async (req, res) => {
  try {
    const {
      post_id,
      author_email,
